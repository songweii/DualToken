...
class VQEmbedding_ema(nn.Embedding):
    """VQ embedding module with ema update."""

    def __init__(self, n_embed, embed_dim, ema=True, decay=0.99, restart_unused_codes=True, eps=1e-5, _weight=None):
        super().__init__(n_embed + 1, embed_dim, padding_idx=n_embed, _weight=_weight)
        ...
        if self.ema:
            ...
            self.register_buffer("embed_ema", self.weight[:-1, :].detach().clone().float())
    ...
    def compute_distances(self, inputs):
        ...
        inputs_flat = inputs.reshape(-1, embed_dim).float()
        codebook_t = codebook_t.float()
    ...
    @torch.no_grad()
    def _update_buffers(self, vectors, indexs):
        ...
        vecs = vectors.reshape(-1, embed_dim).float()
        idxs = indexs.reshape(-1)
        
        n_vectors = vecs.shape[0]
        n_total_embed = n_embed

        one_hot_idxs = vecs.new_zeros(n_total_embed, n_vectors)
        one_hot_idxs.scatter_(
            dim=0,
            index=idxs.unsqueeze(0),
            src=vecs.new_ones(1, n_vectors)
        )

        cluster_size = one_hot_idxs.sum(dim=1)
        vectors_sum_per_cluster = one_hot_idxs @ vecs
        ...
        if self.restart_unused_codes:
            if n_vectors < n_embed:
                vecs = self._tile_with_noise(vecs, n_embed)
            n_vectors = vecs.shape[0]
            _vectors_random = vecs[torch.randperm(n_vectors, device=vecs.device)][:n_embed]
    ...
    def forward(self, inputs):
        ...
        embeds = super().forward(embed_idxs)
...
class RQBottleneck_ema(nn.Module):
    """
    ...
    def __init__(self,
        ...
        if self.shared_codebook:
            codebook0 = VQEmbedding_ema(self.n_embed[0], 
                                        embed_dim, 
                                        decay=self.decay[0], 
            ...
            codebooks = [VQEmbedding_ema(self.n_embed[idx], 
                                         embed_dim, 
        ...
        # —— 缓存：用于在 accumulation 结束时统一做 EMA —— #
        if self.shared_codebook:
            self._ema_vecs_shared: List[torch.Tensor] = []
            self._ema_idxs_shared: List[torch.Tensor] = []
        else:
            self._ema_vecs: List[List[torch.Tensor]] = [[] for _ in range(self.code_shape[-1])]
            self._ema_idxs: List[List[torch.Tensor]] = [[] for _ in range(self.code_shape[-1])]
    ...
    @torch.no_grad()
    def _cache_for_ema(self, level: int, residual: torch.Tensor, idxs: torch.Tensor):
        """
        把本 micro-batch 的残差向量与索引缓存起来，等 accumulation 结束统一做 EMA。
        residual: (B, h, w, embed_dim)
        idxs:     (B, h, w)
        """
        if self.shared_codebook:
            self._ema_vecs_shared.append(residual.reshape(-1, residual.size(-1)).detach())
            self._ema_idxs_shared.append(idxs.reshape(-1).detach())
        else:
            self._ema_vecs[level].append(residual.reshape(-1, residual.size(-1)).detach())
            self._ema_idxs[level].append(idxs.reshape(-1).detach())
    ...
    def quantize(self, x):
        ...
        for i in range(self.code_shape[-1]):
            ...
            # 训练期仅缓存，不更新 codebook
            if self.training:
                self._cache_for_ema(i, residual_feature, code)
    ...
    def flush_ema_updates(self):
        ...
        if self.shared_codebook:
            if len(self._ema_vecs_shared):
                vecs = torch.cat(self._ema_vecs_shared, dim=0)
                idxs = torch.cat(self._ema_idxs_shared, dim=0)
                cb = self.codebooks[0]
                cb._update_buffers(vecs, idxs)      # 统计 + all_reduce
                cb._update_embedding()              # 写回权重
                self._ema_vecs_shared.clear()
                self._ema_idxs_shared.clear()
        else:
            for d, cb in enumerate(self.codebooks):
                if len(self._ema_vecs[d]):
                    vecs = torch.cat(self._ema_vecs[d], dim=0)
                    idxs = torch.cat(self._ema_idxs[d], dim=0)
                    cb._update_buffers(vecs, idxs)
                    cb._update_embedding()
                    self._ema_vecs[d].clear()
                    self._ema_idxs[d].clear()
